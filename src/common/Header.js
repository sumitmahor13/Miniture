...
const Header = () => {
  ...
  return (
    <>
      <div
        className={`${sticky ? "header sticky top-0 z-50 shadow-xl" : ""}`}
      >
        <div className="flex flex-wrap justify-between items-center w-10/12 m-auto">
          <div>
            <Link to="/"><div class="logo">min<span className="span">i</span>ture</div></Link>
          </div>
          <div className="md:flex flex-wrap text-base gap-3 font-semibold py-3">
            {navbar.map((nav, key) => (
